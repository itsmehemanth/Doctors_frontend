...
const Home = () => {
  ...
  useEffect(() => {
    const fetchPatientsAndDoctors = async () =>{
      try{
        const patiensResponse = await axios.get('https://backendhospital-ji3g.onrender.com/patients');
        const doctorsResponse = await axios.get('https://backendhospital-ji3g.onrender.com/doctors');
        setPatients(patiensResponse.data);
  ...
  return (
    ...
    <div>
      ...
      <select onChange={handleDoctorChange}>
        <option value={null}>All Doctor</option>
        {doctors.map((doctor) =>(
          <option key={doctor.id} value={doctor.id}>
            {doctor.name} - {doctor.specialzation}
          </option>
        ))}
      </select>
      <table>
        ...
        <tbody>
         {filteredpatients.map((patient) => (
          <tr key={patient.id}>
            ...
            <td>{patient.doctor?.name}-{patient.doctor?.specialzation}</td>
            <td>
